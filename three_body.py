...
def plot(state, canvas, color_factor):
	...
	disk = make_disk()
	for mass in state:
		# the unit square is stretched to fit the canvas, so if any mass is too close to the edge then don't
		# try to draw it.
		...
		y = int(y*height)
		r = 3 
		try: # should not need this try clause.  should be caught in the if.. above.
